...
static const char *executable_name = "dwmblocks";

void signal_dwmblocks(void) {
    Number clip;
    DIR *proc_dir;
    struct dirent *proc_dirent;
    pid_t pid;

    if (!(clip.str = getenv("CLIPBOARD"))) {
        fprintf(stderr, "CLIPBOARD environment variable not set.\n");
        return;
    }
    if ((clip.num = atoi(clip.str)) < 10) {
        fprintf(stderr, "Invalid CLIPBOARD environment variable.\n");
        return;
    }
    ...
    while ((proc_dirent = readdir(proc_dir))) {
        if ((pid = check_pid(proc_dirent->d_name))) {
            kill(pid, SIGRTMIN+clip.num);
            break;
        }
    }
...
pid_t check_pid(char *name) {
    ...
    if (!strcmp(pbuf, executable_name)) {
        fclose(stat_file);
        return pid;
    }
