...
// Defines HAS_UINT128 and uint128 if applicable.
...
static const uint64 DOUBLE_POW5_INV_SPLIT2[15][2] = {
...
static const uint32 POW5_INV_OFFSETS[22] = {
...
static const uint64 DOUBLE_POW5_SPLIT2[13][2] = {
...
static const uint32 POW5_OFFSETS[21] = {
...
static const uint64 DOUBLE_POW5_TABLE[POW5_TABLE_SIZE] = {
...
static inline void double_computePow5(const uint32 i, uint64* const result) {
  const uint32 base = i / POW5_TABLE_SIZE;
  const uint32 base2 = base * POW5_TABLE_SIZE;
  const uint32 offset = i - base2;
  const uint64* const mul = DOUBLE_POW5_SPLIT2[base];
  if (offset == 0) {
    result[0] = mul[0];
    result[1] = mul[1];
    return;
  ...
  const uint128 shiftedSum = (b0 >> delta) + (b2 << (64 - delta)) + ((POW5_OFFSETS[i / 16] >> ((i % 16) << 1)) & 3);
  result[0] = (uint64) shiftedSum;
  result[1] = (uint64) (shiftedSum >> 64);
}

// Computes 5^-i in the form required by Ryu, and stores it in the given pointer.
static inline void double_computeInvPow5(const uint32 i, uint64* const result) {
  const uint32 base = (i + POW5_TABLE_SIZE - 1) / POW5_TABLE_SIZE;
  const uint32 base2 = base * POW5_TABLE_SIZE;
  const uint32 offset = base2 - i;
  const uint64* const mul = DOUBLE_POW5_INV_SPLIT2[base]; // 1/5^base2
  if (offset == 0) {
    result[0] = mul[0];
    result[1] = mul[1];
    return;
  }
  const uint64 m = DOUBLE_POW5_TABLE[offset]; // 5^offset
  const uint128 b0 = ((uint128) m) * (mul[0] - 1);
  const uint128 b2 = ((uint128) m) * mul[1]; // 1/5^base2 * 5^offset = 1/5^(base2-offset) = 1/5^i
  const uint32 delta = pow5bits(base2) - pow5bits(i);
  assert(i / 16 < sizeof(POW5_INV_OFFSETS) / sizeof(POW5_INV_OFFSETS[0]));
  const uint128 shiftedSum =
...
static inline void double_computePow5(const uint32 i, uint64* const result) {
  ...
  uint64 high0;
  const uint64 low0 = umul128(m, mul[0], &high0);
...
static inline void double_computeInvPow5(const uint32 i, uint64* const result) {
  ...
  const uint64 low1 = umul128(m, mul[1], &high1);
  uint64 high0;
  const uint64 low0 = umul128(m, mul[0] - 1, &high0);
  const uint64 sum = high0 + low1;
  if (sum < high0) {
    ++high1; // overflow into high1
  }
